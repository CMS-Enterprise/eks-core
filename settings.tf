...
#     data.aws_ec2_managed_prefix_list.cmscloud_public_pl.id,
  ...
  config_settings = {
    ...
    drop_namespaces        = "(${join("|", var.fb_drop_namespaces)})"
    log_filters            = "(${join("|", var.fb_log_filters)})"
    additional_log_filters = "(${join("|", var.fb_additional_log_filters)})"
    kube_namespaces        = var.fb_kube_namespaces
  }
